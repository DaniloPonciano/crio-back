...
public class EventoRepositoryTests {
    ...
    void findEventoByNameDocumentCase1() {
        ...
        EventoResponseDTO data = new EventoResponseDTO(null, titulo, descricao, inicio, 
        ...
        Optional<Evento> result = this.eventoRepository.findByLocal(local);

        assertThat(result.isPresent()).isTrue();
        assertThat(result.get().getLocal()).isEqualTo(local);
    }

    @Test
    @DisplayName("Should not get Evento from DB when Evento not exists")
    void findEventoByNameDocumentCase2() {
        String name = "EventoInexistente"; // nome da categoria que estamos procurando

        Optional<Evento> result = this.eventoRepository.findByLocal(name);

        assertThat(result.isEmpty()).isTrue();
    }

    private Evento createEvento(EventoResponseDTO data) {
